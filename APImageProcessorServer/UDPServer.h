...
private:
	...
	bool HasRequestTimedOut(high_resolution_clock::time_point& lastImagePayloadRecdTime, const ushort& timeoutDuration);
	short SendMissingPayloadSeqNumbersToClient(std::map<u_short, std::string>& imagePayloadSeqMap, const u_short& expectedNumberOfPayloads, 
	...
	ushort DrainQueue(std::queue<std::string>& clientQueue, std::string& imageSizeString);
